...
var trivia = {
    ...
    guessChecker: function () {
        ...
        if ($(this).text() === currentAnswer) {
            ...
            $('#results').html('<h2>The Game is Over!</h2>');
        }
        else {
            trivia.incorrect++;
            this.questionCounter++; 
    ...
    nextQuestion: function () {
        ...
        $.each(trivia.options[questionKey], function (key, values) {
            $('#options').append(
                // very efficient way to make ANY element in jquery 
                $('<button>').addClass("option-btn").text(values).attr("data-id", key)
            )   
        })
        console.log(this.questionCounter, questionArray, questionKey, questionText);
        //30 second timer
        trivia.timer = 30;
        $('#timer').text(trivia.timer);
...
function createOptions() {
    $.each(trivia.options, function (key, values) {
